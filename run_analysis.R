...
## Step 0: load the required special package: reshape2 (to use melt fuction and dcast function) melt function and dcast function will be used in Step 5
...
## only select those columns/measurements on the mean and standard deviation make a new data frame called joindatanew
...
## remove the bad characters such as "()", "-" in colnames, also lower the case if possible in order to avoid any unnecessary errors in later analysis
...
## Create a new activitylabel vector containing descriptive activity names with the length of rows of joinlabel[, 1] 
...
## Reshape the data: generate skinny data via melt function Melt the cleadata data set before decasting
